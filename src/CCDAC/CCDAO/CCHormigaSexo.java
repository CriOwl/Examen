...
public class CCHormigaSexo extends CCDataHelperSqlite implements IDAO<CCHormigaSexoDTO>{
    @Override
    public boolean created(CCHormigaSexoDTO entity)throws Exception{
        try {
            String query="INSERT INTO CCSexo (Name,Estado) VALUES(?,?)";
    ...
    @Override
    public boolean update(CCHormigaSexoDTO entity)throws Exception{
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        LocalDateTime now = LocalDateTime.now();
        try {
            String query="UPDATE CCSexo SET"
                        +",Name=?, "
                        +",Estado=?, "
                        +",FechaActua=? WHERE IdSexo = ? ";
            Connection cone = opConnection();
            PreparedStatement pst=cone.prepareStatement(query);
            pst.setString(1, entity.getName());
            pst.setString(2, entity.getEstado());
    ...
    public List<CCHormigaSexoDTO> ccReadBox(){
        List<CCHormigaSexoDTO> ccTabla=new ArrayList<>();
        String ccQuery="SELECT "
                        +"IdSexo "      
                        +",Name "         
                        +"FROM CCSexo WHERE Estado='A'";  
        try {
              Connection conec=opConnection();
              Statement stm=conec.createStatement();
              ResultSet rs=stm.executeQuery(ccQuery);
              while(rs.next()){
                CCHormigaSexoDTO registro=new CCHormigaSexoDTO(
                                    rs.getInt(1),
                                    rs.getString(2));
